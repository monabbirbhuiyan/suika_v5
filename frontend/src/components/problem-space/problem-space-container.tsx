...
import { Fragment, ProblemSpace } from "@/types/canva";
...
const ProblemSpaceContainer = ({ problemSpace }: Props) => {
  ...
  const [editingSpace, setEditingSpace] = useState<ProblemSpace | null>(null);
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deletingSpace, setDeletingSpace] = useState<ProblemSpace | null>(null);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/problem-spaces/${deletingSpace.id}`,
        {
          method: "DELETE",
        },
      );
      ...
      toast.add({
        type: "error",
        description: "Failed to delete problem space.",
        priority: "high",
      });
    } finally {
      setIsDeleting(false);
    ...
    <div className="p-8 max-w-5xl w-full mx-auto">
      ...
      >
        <div className="flex items-center justify-end">
          <Button
            ...
            className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium hover:opacity-90 transition-opacity shrink-0"
          >
      ...
      {spaces.length > 0 ? (
        ...
        >
          {spaces.map((space) => {
            ...
            const handleEditClick = (
              e: React.MouseEvent,
              targetSpace: ProblemSpace,
            ) => {
              e.preventDefault();
              e.stopPropagation();
              setEditingSpace(targetSpace);
              setEditOpen(true);
            };
            ...
            return (
              <Link
                ...
                className="rounded-xl bg-card border p-6 text-left hover:border-primary/40 transition-all group block relative"
              >
                ...
                <div className="mt-3">
                  <div className="h-2 w-full overflow-hidden rounded-full bg-border/60 flex">
                    {counts.map((item) => {
                      if (item.count === 0 || fragmentCount === 0) return null;
          ...
          <button
            onClick={() => setCreateOpen(true)}
            className="rounded-xl border-2 border-dashed border-border p-6 flex flex-col items-center justify-center gap-3 text-muted-foreground hover:text-primary hover:border-primary/40 transition-all group min-h-40"
          >
            <div className="rounded-full bg-card p-3 group-hover:bg-primary/15 transition-colors">
        ...
        <motion.div
          ...
          transition={{ duration: 0.5, delay: 0.1 }}
        >
          <div className="rounded-full bg-card p-5 border">
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-sm leading-relaxed">
            A problem space is a container for legal inquiries and facts. Create
            your first one to map arguments and verify logic with Z3.
          </p>
          <button
            onClick={() => setCreateOpen(true)}
            className="mt-6 inline-flex items-center gap-2 rounded-full bg-primary text-primary-foreground px-6 py-3 text-sm font-medium hover:opacity-90 transition-opacity"
          >
            <Plus className="h-4 w-4" />
      ...
      <EditProblemSpaceForm
        ...
        userId={editingSpace?.userId || editingSpace?.user_id || ""}
      />
      ...
      {deleteOpen && deletingSpace && (
        ...
        >
          <div
            className="bg-background rounded-xl border border-border p-6 shadow-xl max-w-md w-full mx-4"
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-6">
              Are you sure you want to delete &quot;{deletingSpace.title}&quot;?
              This permanently removes all associated fragments, nodes, and
